// ==========================================
// 1. GLOBAL STATE & KONEKSI API BACKEND
// ==========================================
let products = [];
...
// Base URL Backend Railway kamu
const BASE_URL = 'https://anamac-backend-production.up.railway.app/api';

// AWAL MUASAL DATA (Koneksi langsung ke Database)
async function fetchProducts() {
    try {
        // Tembak API Backend kita!
        const res = await fetch(`${BASE_URL}/products`); 
        
        // Cek kalau responnya gagal
        if (!res.ok) throw new Error("Gagal mengambil data dari API");

        // Ambil data JSON dari backend
        const data = await res.json();
        
        // Masukkan data dari database ke state frontend
        products = data; 
        currentDisplayData = [...products];
        
        // Render ulang tampilannya
        displayPage(1);
        updateUIStats();
        checkAuthUI();
        
        console.log("🔥 Sukses narik data dari database Railway!", products);
    } catch (e) {
        console.error("Waduh, error narik data API nih:", e);
        // Fallback kalau API mati, biar web nggak blank
        const grid = document.getElementById('product-grid');
        if (grid) grid.innerHTML = "<p class='col-span-full text-center text-slate-500 py-10'>Gagal memuat produk dari database. Coba refresh halaman.</p>";
    }
}

// ==========================================
// 2. RENDER ENGINE (Tampilan Grid Depan)
// ==========================================
    ...
    content.innerHTML = `
        <div class="space-y-6">
            ...
            <div class="px-2">
                ...
                <h3 class="text-2xl font-black mt-4 dark:text-white">${p.name}</h3>
...
// ==========================================
// 6. UI UTILITIES (TOAST, PAGINATION, THEME)
// ==========================================
