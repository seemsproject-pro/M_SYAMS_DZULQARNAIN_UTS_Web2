...
let currentSearchTerm = ''; // Menyimpan kata kunci pencarian
let currentPage = 1;
...
// ==========================================
// 2. CORE FILTER ENGINE (UNIFIED LOGIC)
// ==========================================
...
function applyFilters() {
    ...
    // 3. Filter & Sort berdasarkan Harga
    const priceMode = document.getElementById('priceFilter')?.value || 'all';
    
    if (priceMode === 'low') {
        filtered.sort((a, b) => a.price - b.price); // Termurah
    } else if (priceMode === 'high') {
        filtered.sort((a, b) => b.price - a.price); // Termahal
    } else if (priceMode === 'under5') {
        filtered = filtered.filter(p => p.price < 5000000); // < 5 Juta
    } else if (priceMode === 'over5') {
        filtered = filtered.filter(p => p.price >= 5000000); // > 5 Juta
    }

    currentDisplayData = filtered;
    displayPage(1); // Setiap kali filter berubah, balik ke halaman 1
}
...
// ==========================================
// 3. RENDER ENGINE (GRID DEPAN - FIX MOBILE)
// ==========================================
...
function renderProducts(items) {
    ...
    if (items.length === 0) {
        grid.innerHTML = `<div class="col-span-full py-20 text-center opacity-40 italic font-medium">Hardware tidak ditemukan...</div>`;
        return;
    }
        ...
        grid.innerHTML += `
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] p-4 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 hover:shadow-2xl transition-all duration-500 group">
                ...
                <div class="space-y-3 px-2">
                    ...
                    <h3 class="font-display font-bold text-lg line-clamp-1 hover:text-blue-600 transition-colors cursor-pointer dark:text-white" onclick="toggleDetailSidebar(${p.id})">${p.name}</h3>
                    ...
                    <div class="pt-4 flex justify-between items-center border-t border-slate-50 dark:border-slate-700 mt-2">
                        <span class="text-xl font-display font-black text-blue-600 dark:text-blue-400">Rp ${Number(p.price).toLocaleString('id-ID')}</span>
                        <button onclick="addToCart(${p.id})" 
                                class="bg-slate-900 dark:bg-blue-600 text-white h-12 w-12 rounded-2xl flex items-center justify-center shadow-md active:scale-90 transition-all">
                            🛒
                        </button>
                    </div>
                </div>
...
function renderWishlistInDrawer() {
    ...
    container.innerHTML = wished.length ? wished.map(p => `
        <div class="flex items-center gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border dark:border-slate-700 mb-2">
            ...
            <div class="flex-1"><h4 class="font-bold text-sm line-clamp-1">${p.name}</h4><p class="text-blue-600 font-black text-xs">Rp ${p.price.toLocaleString('id-ID')}</p></div>
...
function renderDetailInDrawer(p) {
    ...
    content.innerHTML = `
        <div class="space-y-6 p-1">
            ...
            <div class="px-2">
                <span class="text-[10px] font-black uppercase text-blue-600 bg-blue-600/10 px-3 py-1 rounded-full">${p.category}</span>
            ...
            <div class="pt-6 px-2">
                ...
                <div class="space-y-4 mb-10">
                    ${reviews.length ? reviews.map(r => `<div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-3xl border dark:border-slate-700 text-xs"><div class="flex justify-between mb-1"><span class="font-bold text-blue-600 uppercase italic">${r.userName}</span><span class="text-yellow-500">${'⭐'.repeat(r.rating)}</span></div><p class="italic text-slate-600 dark:text-slate-300">"${r.comment}"</p></div>`).join('') : '<p class="text-center opacity-40 py-10 text-xs">Belum ada ulasan.</p>'}
                </div>
            </div>
        </div>`;
    ...
    if (user) {
        formArea.innerHTML = `<div class="space-y-4"><div class="flex gap-2 justify-center text-2xl">${[1,2,3,4,5].map(n => `<button onclick="submitReview(${p.id}, ${n})" class="hover:scale-125 transition-all">⭐</button>`).join('')}</div><div class="flex gap-2"><input type="text" id="reviewInput" placeholder="Komentar..." class="flex-1 p-3 rounded-2xl border-none bg-white dark:bg-slate-700 focus:ring-2 focus:ring-blue-600 text-sm outline-none"><button onclick="submitReview(${p.id}, 5)" class="bg-blue-600 text-white px-5 rounded-2xl font-bold text-xs uppercase shadow-lg">Kirim</button></div></div>`;
    } else {
        formArea.innerHTML = `<p class="text-center text-[10px] text-slate-500 italic uppercase">Silakan <a href="login.html" class="text-blue-600 font-bold underline">Login</a> untuk mereview.</p>`;
    }
}

// ==========================================
// 6. AUTH & BUSINESS LOGIC
// ==========================================
    ...
    if (user && area) {
        area.innerHTML = `
            <div class="flex items-center gap-2 bg-white dark:bg-slate-800 p-1.5 pr-2 rounded-2xl border dark:border-slate-700 shadow-sm">
                <div class="w-8 h-8 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-xs uppercase italic">${user.name[0]}</div>
...
function toggleWishlist(id, fromDrawer = false) {
    ...
    updateUIStats(); applyFilters(); // Update tampilan grid dengan filter aktif
...
function submitReview(productId, rating) {
    ...
    showToast("Terima kasih! ⭐"); toggleDetailSidebar(productId); applyFilters();
}

// ==========================================
// 7. UI UTILITIES (TOAST, PAGINATION, THEME)
// ==========================================
...
function renderPagination() {
    ...
    for (let i = 1; i <= total; i++) {
        ...
        btn.className = `w-12 h-12 rounded-2xl font-bold transition-all ${currentPage === i ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'bg-white dark:bg-slate-800 text-slate-400 hover:bg-slate-100'}`;
